...
namespace osc {
    ...
    // SBT record for plane's hitgroup
    struct __align__(OPTIX_SBT_RECORD_ALIGNMENT) HitgroupRecord2
    {
        __align__(OPTIX_SBT_RECORD_ALIGNMENT) char header[OPTIX_SBT_RECORD_HEADER_SIZE];
        PlaneSBTData data;
    };
        ...
        : model(model)
    {
        ...
        for (auto light : model->triangleLights) {
            launchParams.lights.a[idx] = light.a;
            launchParams.lights.b[idx] = light.b;
            launchParams.lights.c[idx] = light.c;
    ...
    void SampleRenderer::createHitgroupPrograms()
    {
        ...
        //-------------------------------------------------------
        // for planes hitgroup
        hitgroupPlanes.resize(RAY_TYPE_COUNT);      // two types of ray for planes
        char log2[2048];
        ...
        pgDesc2.hitgroup.entryFunctionNameCH = "__closesthit__radiance_plane";
        pgDesc2.hitgroup.moduleAH = module;
        pgDesc2.hitgroup.entryFunctionNameAH = "__anyhit__radiance";
        pgDesc2.hitgroup.moduleIS = module;
        pgDesc2.hitgroup.entryFunctionNameIS = "__intersection__plane";
        ...
        OPTIX_CHECK(optixProgramGroupCreate(optixContext,
            ...
            &pgOptions2,
            log2, &sizeof_log2,
            &hitgroupPlanes[RADIANCE_RAY_TYPE]));

        // for shadow rays
        pgDesc2.hitgroup.entryFunctionNameCH = "__closesthit__shadow";
        pgDesc2.hitgroup.entryFunctionNameAH = "__anyhit__shadow";
        ...
        OPTIX_CHECK(optixProgramGroupCreate(optixContext,
            ...
            &pgOptions2,
            log2, &sizeof_log2,
            &hitgroupPlanes[SHADOW_RAY_TYPE]));

        if (sizeof_log2 > 1) PRINT(log);
    ...
    void SampleRenderer::buildSBT()
    {
        ...
        for (int i = 0; i < numPlanes; i++) {
            for (int rayID = 0; rayID < RAY_TYPE_COUNT; rayID++) {
                auto plane = model->planes[i];
                HitgroupRecord2 rec;
                OPTIX_CHECK(optixSbtRecordPackHeader(hitgroupPlanes[rayID], &rec));
                rec.data.color = plane->color;
                if (plane->isReflective) {
                    rec.data.isReflective = true;
                    rec.data.fuzzy = plane->fuzzy;
                }
                if (plane->isRefractive) {
                    rec.data.isRefractive = true;
                    rec.data.ior = plane->ior;
                }

                hitgroupPlaneRecords.push_back(rec);
            }
        }

        std::cout << hitgroupPlaneRecords.size() << std::endl;
        if (hitgroupPlaneRecords.size() > 0) {
            hitgroupRecordsBuffer2.alloc_and_upload(hitgroupPlaneRecords);
            sbt2.hitgroupRecordBase = hitgroupRecordsBuffer2.d_pointer();
            sbt2.hitgroupRecordStrideInBytes = sizeof(HitgroupRecord2);
